...
{
        if (!ops || !ops->app_mem_malloc || !ops->app_mem_free || !ops->app_get_list) {
...
static struct core_timer_list *app_get_list(void)
{
        if (!_app_timer_ops.app_get_list) {
                return NULL;
        }
        return _app_timer_ops.app_get_list();
}
...
static void _callback(struct core_timer *core, void *user_data)
{
        ...
        if (timer->mode == TIMER_PERIODIC) {
                ...
                luna_timer_insert(app_get_list(), &timer->core);
        }
...
int app_timer_start(struct app_timer *timer)
{
        ...
        return luna_timer_insert(app_get_list(), &timer->core);
}
...
int app_timer_stop(struct app_timer *timer)
{
        ...
        struct core_timer *removed = luna_timer_remove(app_get_list(), &timer->core);
